...
...   results += segment["cellids"].tolist()
...
\end{python}
...
\noindent The optional argument \texttt{head=} above sets the specific-discharge in inactive or dry cells to NaN. The \texttt{get\_specific\_discharge()} method also includes functionality to plot specific discharge values at the cell \texttt{"centers"} (default), \texttt{"faces"}, or \texttt{"vertices"}; currently, \texttt{"faces"}, or \texttt{"vertices"} are only implemented for structured grids. 
